...
class GenericTrainingManager:
    ...
    def train(self):
        ...
        for num_epoch in range(self.latest_epoch+1, nb_epochs):
            ...
            num_authors = len(self.dataset.train_loader.batch_sampler.author_dict)
    ...
    def evaluate(self, set_name, **kwargs):
        ...
        with tqdm(total=len(loader.batch_sampler.author_dict)) as pbar:
    ...
    def predict(self, custom_name, sets_list, metric_names, output=False):
        ...
        total_confidence_score_sum = 0.0
        total_confidence_score_count = 0
        # Set models in eval mode
        for model_name in self.models.keys():
            self.models[model_name].eval()
        ...
        with tqdm(total=len(loader.batch_sampler.author_dict)) as pbar:#loader.dataset
        #with tqdm(total=len(loader.dataset)) as pbar:
            ...
            for ind_batch, batch_data in enumerate(loader):
                ...
                batch_values = self.test_batch(batch_data, metric_names)  #evaluate_batch
                batch_confidence_scores = batch_values["confidence_score"]  # Assuming this is a list of lists
                batch_sum = sum([sum(sublist) for sublist in batch_confidence_scores])
                batch_count = sum([len(sublist) for sublist in batch_confidence_scores])
                total_confidence_score_sum += batch_sum
                total_confidence_score_count += batch_count
        ...
        if total_confidence_score_count > 0:
            overall_average_confidence_score = total_confidence_score_sum / total_confidence_score_count
            print("Overall Average Confidence Score:", overall_average_confidence_score)
        else:
            print("No confidence scores available to calculate an average.")
