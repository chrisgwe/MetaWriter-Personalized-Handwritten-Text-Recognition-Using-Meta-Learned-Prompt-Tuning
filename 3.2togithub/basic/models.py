...
from basic.decoder1 import FCNDecoder
...
class ConvBlock(Module):
    ...
    def forward(self, x):
        ...
        if x.size(2) == 1 and x.size(3) == 1:  
            norm_layer = LayerNorm(x.size(1))
            x = norm_layer(x)
        else:
            norm_layer1 = InstanceNorm2d(x.size(1), eps=0.001, momentum=0.99, track_running_stats=False)
            x = norm_layer1(x)
