...
def get_list_augmenters(img, aug_configs, fill_value):
    ...
    for aug_config in aug_configs:
        ...
        if aug_config["type"] == "dpi":
            ...
            # Adjust max_kernel_h and max_kernel_w if they are smaller than min_kernel
            max_kernel_h = max(max_kernel_h, aug_config["min_kernel"])
            max_kernel_w = max(max_kernel_w, aug_config["min_kernel"])
